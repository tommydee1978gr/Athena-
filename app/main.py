...
async def integrations_page(request: Request, user=Depends(current_user)):
    ...
    if user["role"] == "admin":
        ...
        <h3>Unraid server (ο ίδιος ο server — μόνο admin)</h3><p><small><code>unraid-api apikey --name ATHENA --create --roles VIEWER</code> στο ίδιο το Unraid (SSH) για να πάρεις το key. Σημείωση: το port 443 δείχνει μόνο localhost — βάλε plain http στο port 80.</small></p><label>Base URL<input name='unraid_base_url' value='{esc(unraid.get('base_url',''))}' placeholder='http://192.168.1.2'></label><label>API key<input name='unraid_api_key' type='password'></label>{secret_note}
...
async def integrations_config(request: Request, csrf: str = Form(...), user=Depends(require_admin)):
    ...
    mapping = {
        ...
        "elevenlabs": {
            "api_key": form.get("elevenlabs_api_key"),
            "default_voice_id": form.get("elevenlabs_default_voice_id"),
            "model_id": form.get("elevenlabs_model_id") or "eleven_v3",
            "stability": float(form.get("elevenlabs_stability") or 0.45),
        ...
        "unraid": {"base_url": form.get("unraid_base_url"), "api_key": form.get("unraid_api_key")},
    }
...
async def graph_page(request: Request, user=Depends(current_user)):
    ...
    <div id='askBar' class='panel'>
      <button id='wakeToggle' title='Ακούει πάντα για "Αθηνά" — πάτα για σίγαση'>👂</button>
