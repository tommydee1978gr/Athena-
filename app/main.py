...
async def integrations_page(request: Request, user=Depends(current_user)):
    ...
    if user["role"] == "admin":
        ...
        <h3>ElevenLabs (voice — preferred over local Whisper/Piper when set)</h3><p><small>Free tier is enough to try it. Χρησιμοποιείται αυτόματα για STT/TTS όποτε υπάρχει API key εδώ· χωρίς αυτό πέφτει στο τοπικό Whisper/Piper.</small></p><label>API key<input name='elevenlabs_api_key' type='password'></label><label>Default voice ID (προαιρετικό)<input name='elevenlabs_default_voice_id' value='{esc(elevenlabs.get("default_voice_id",""))}' placeholder='21m00Tcm4TlvDq8ikWAM'></label>{secret_note}
...
async def integrations_config(request: Request, csrf: str = Form(...), user=Depends(require_admin)):
    ...
    mapping = {
        ...
        "elevenlabs": {"api_key": form.get("elevenlabs_api_key"), "default_voice_id": form.get("elevenlabs_default_voice_id")},
